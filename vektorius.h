...
/**
 * @brief Dinaminio masyvo realizacija, panaši į std::VEKTORIUS.
 *
...
public:
     ...
     */
    VEKTORIUS() : m_data(nullptr), capacity_(0), length(0) {}
     ...
     */
    VEKTORIUS(const VEKTORIUS &other) : m_data(nullptr), capacity_(0), length(0)
    {
        *this = other;
    }
     ...
     *
     * @param other Kopijuojamas vektorius.
     * @return VEKTORIUS& Nuoroda į šį vektorių po priskyrimo.
     */
     ...
     */
    VEKTORIUS(VEKTORIUS &&other) noexcept : m_data(nullptr), capacity_(0), length(0)
    {
        *this = std::move(other);
    }
     ...
     *
     * @param other Perkeliamas vektorius.
     * @return VEKTORIUS& Nuoroda į šį vektorių po perkelimo priskyrimo.
     */
     ...
     */
    friend std::ostream &operator<<(std::ostream &out, const VEKTORIUS &vec)
    {
        out << "[";
        if (!vec.empty())
        {
    ...
    void rotate(iterator start, iterator middle, iterator end)
    {
        ...
        VEKTORIUS<T> temp(leftLength + rightLength); // Temporary VEKTORIUS to store rotated elements
